...
if(!c7){c7=cU(e,new RegExp("(^|"+cy+")"+e+"("+cy+"|$)"))
...
}),last:cF(function(e,c8,c7){return[c8-1]
}),eq:cF(function(e,c8,c7){return[c7<0?c7+c8:c7]
}),even:cF(function(e,c9,c8){for(var c7=0;
c7<c9;
c7+=2){e.push(c7)
}return e
}),odd:cF(function(e,c9,c8){for(var c7=1;
c7<c9;
c7+=2){e.push(c7)
}return e
...
c8=[];
e=cL.preFilter;
while(dc){if(!c7||(db=b5.exec(dc))){if(db){dc=dc.slice(db[0].length)
}c8.push(dd=[])
}c7=false;
if((db=cH.exec(dc))){dd.push(c7=new b4(db.shift()));
dc=dc.slice(c7.length);
c7.type=db[0].replace(cT," ")
}for(de in cL.filter){if((db=cG[de].exec(dc))&&(!e[de]||(db=e[de](db,b8,true)))){dd.push(c7=new b4(db.shift()));
dc=dc.slice(c7.length);
c7.type=de;
c7.matches=db
}}if(!c7){break
...
}return c1(function(dn,dk,de,dm){if(dn&&db){return
...
if(da){da(di,dj,de,dm)
}if(c9){dh=cv(dj,dp);
c9(dh,[],de,dm);
dl=dh.length;
while(dl--){if((dg=dh[dl])){dj[dp[dl]]=!(di[dp[dl]]=dg)
}}}if(dn){dl=c8&&dj.length;
while(dl--){if((dg=dj[dl])){dn[df[dl]]=!(dk[df[dl]]=dg)
}}}else{dj=cv(dj===dk?dj.splice(dd,dj.length):dj);
if(db){db(null,dk,dj,dm)
}else{cW.apply(dk,dj)
}}})
...
function cp(c7,db,da,c8){var c9=0,e=db.length;
for(;
c9<e;
c9++){cQ(c7,db[c9],da,c8)
}return da
...
if(!cm(de)&&!e.test(dg)&&(!c7||!c7.test(dg))){try{var dd=c9.call(de,dg);
...
if(b8.crossDomain==null){cd=aT.exec(b8.url.toLowerCase())||false;
...
(function(i){var a=(typeof setImmediate==="function"&&setImmediate)||function(m){setTimeout(m,1)
};
function g(n,m){return function(){n.apply(m,arguments)
}
}var c=Array.isArray||function(m){return Object.prototype.toString.call(m)==="[object Array]"
};
function h(m){if(typeof this!=="object"){throw new TypeError("Promises must be constructed via new")
}if(typeof m!=="function"){throw new TypeError("not a function")
}this._state=null;
this._value=null;
this._deferreds=[];
k(m,g(j,this),g(l,this))
}function d(m){var n=this;
if(this._state===null){this._deferreds.push(m);
return
}a(function(){var o=n._state?m.onFulfilled:m.onRejected;
if(o===null){(n._state?m.resolve:m.reject)(n._value);
return
}var p;
try{p=o(n._value)
}catch(q){m.reject(q);
...
}function j(n){try{if(n===this){throw new TypeError("A promise cannot be resolved with itself.")
}if(n&&(typeof n==="object"||typeof n==="function")){var o=n.then;
if(typeof o==="function"){k(g(o,n),g(j,this),g(l,this));
return
}}this._state=true;
this._value=n;
e.call(this)
}catch(m){l.call(this,m)
}}function l(m){this._state=false;
this._value=m;
e.call(this)
}function e(){for(var n=0,m=this._deferreds.length;
n<m;
n++){d.call(this,this._deferreds[n])
}this._deferreds=null
}function f(p,m,o,n){this.onFulfilled=typeof p==="function"?p:null;
this.onRejected=typeof m==="function"?m:null;
this.resolve=o;
this.reject=n
}function k(p,q,n){var m=false;
try{p(function(r){if(m){return
}m=true;
q(r)
},function(r){if(m){return
}m=true;
n(r)
})
}catch(o){if(m){return
...
n(o)
}}h.prototype["catch"]=function(m){return this.then(null,m)
};
h.prototype.then=function(o,m){var n=this;
return new h(function(q,p){d.call(n,new f(o,m,q,p))
})
};
h.all=function(){var m=Array.prototype.slice.call(arguments.length===1&&c(arguments[0])?arguments[0]:arguments);
return new h(function(r,q){if(m.length===0){return r([])
}var p=m.length;
function o(t,v){try{if(v&&(typeof v==="object"||typeof v==="function")){var u=v.then;
if(typeof u==="function"){u.call(v,function(w){o(t,w)
},q);
return
}}m[t]=v;
if(--p===0){r(m)
...
h.resolve=function(m){if(m&&typeof m==="object"&&m.constructor===h){return m
}return new h(function(n){n(m)
})
};
h.reject=function(m){return new h(function(o,n){n(m)
})
};
h.race=function(m){return new h(function(q,p){for(var o=0,n=m.length;
o<n;
o++){m[o].then(q,p)
}})
};
h._setImmediateFn=function b(m){a=m
};
if(typeof module!=="undefined"&&module.exports){module.exports=h
}else{if(!i.Promise){i.Promise=h
}}})(this);
window.nz=window.nz||{};
window.nz.ffx=window.nz.ffx||{};
(function(a,b){a.log=function(c){if(/(\?|&)log=true/.test(location.search)){console.log(c)
}};
a.dependencies=a.dependencies||{};
a.dependencies.domLoaded=a.dependencies.domLoaded||{};
a.dependencies.domLoaded.promise=new b.Promise(function(c){addEventListener("DOMContentLoaded",function(){c();
a.log("nz.ffx.dependencies.domLoaded.promise resolved at "+parseInt(performance.now(),10)+" milliseconds")
})
});
a.dependencies.main=a.dependencies.main||{};
a.dependencies.main.promise=new b.Promise(function(d,c){a.dependencies.main.resolve=d;
a.dependencies.main.reject=c
});
a.dependencies.video=a.dependencies.video||{};
a.dependencies.video.promise=new b.Promise(function(d,c){a.dependencies.video.resolve=d;
a.dependencies.video.reject=c
});
a.dependencies.account=a.dependencies.account||{};
a.dependencies.account.promise=new b.Promise(function(d,c){a.dependencies.account.resolve=d;
a.dependencies.account.reject=c
})
}(window.nz.ffx,window));
window.nz=window.nz||{};
window.nz.ffx=window.nz.ffx||{};
window.nz.ffx.name="nz_ffx";
window.nz.ffx.path=window.nz.ffx.path||"assets/js";
(function(b){b.dependencies.scriptsLoaded=b.dependencies.scriptsLoaded||[];
function a(c,e){var d=new window.Promise(function(i,h){var g=window.document.getElementsByTagName("head")[0];
var f=document.createElement("script");
f.src=c;
f.async=e;
if(f.addEventListener){f.addEventListener("load",function(){i(c)
},false)
}else{f.attachEvent("load",function(){i(c)
})
}if(f.addEventListener){f.addEventListener("error",function(){h(c)
},false)
}else{f.attachEvent("error",function(){h(c)
})
}b.log("Loading script "+c);
g.appendChild(f)
});
b.dependencies.scriptsLoaded.push({url:c,promise:d,});
return d
}b.loadScript=function(d,e){var f;
var c=b.dependencies.scriptsLoaded.filter(function(g){return g.url===d
});
if(c.length){f=c[0].promise
}else{f=a(d,e)
}return f
}
}(window.nz.ffx));
...
window.nz.ffx=window.nz.ffx||{};
window.nz.ffx.name="nz_ffx";
(function(c){c.dependencies=c.dependencies||{};
c.dependencies.stylesheetsLoaded=c.dependencies.stylesheetsLoaded||[];
function a(e,d){return e&&d
}function b(f,d){var e=new window.Promise(function(j,i){var h=document.createElement("style");
try{h.type="text/css";
if(h.styleSheet){h.styleSheet.cssText=f
}else{h.appendChild(document.createTextNode(f))
}if(d){h.classList.add(d)
}document.querySelector("head").appendChild(h);
j(f)
}catch(g){i(g,f)
}});
c.dependencies.stylesheetsLoaded.push({urlOrStyles:f,promise:e,});
return e
}c.loadStylesheet=function(e,d){var f;
var g=c.dependencies.stylesheetsLoaded.filter(function(h){return h.urlOrStyles===e
});
if(g.length){f=g[0].promise
}else{if(/^http/.test(e)){f=a(e,d)
}else{f=b(e,d)
}}return f
}
}(window.nz.ffx));
(function(b,c){var a="//players.brightcove.net/videojs-ima3/2/videojs.ima3.min.js";
var d={getBrightcovePlayerUrl:function(e){return"//players.brightcove.net/"+e.accountId+"/"+e.playerId+"_default/index.min.js"
},getVideoScripts:function(f){var e=[];
var g=d.getBrightcovePlayerUrl(f);
e.push(g);
if(f.isLoadIma3){e.push(a)
}return e
},initDesktopArticle:function(h,g,f,e){g.each(function(){var k=h(this);
var j={suppress:k.data("suppress"),playlistId:k.data("articlePlaylistId"),};
var i=e(this).id();
var m=k.data("companionId");
var l=true;
new f(j,i,m,e,l)
})
},initSection:function(h,g,f,e){g.each(function(){var k=h(this);
var j={suppress:k.data("suppress")};
var i=e(this).id();
var m=null;
var l=false;
new f(j,i,m,e,l)
})
},initVideoChannel:function(j,e,i){var g={suppress:false,};
var f="video-channel";
var l="video-channel-companion";
var k=false;
var h=i(f);
new j(g,f,l,i,k);
new e(h)
}};
b.loadVideoHelpers=d
}(window.nz.ffx,window));
(function(b,c){var a;
b.scriptUrls=b.scriptUrls||{};
b.polyfills=b.polyfills||{};
b.polyfills.promises=[];
if("querySelector" in document&&"localStorage" in c&&"addEventListener" in c){b.mustard=true;
if(!c.matchMedia){a=new c.Promise(function(f,e){try{b.scriptUrls.polyfillMatchMedia=(b.scriptUrls.polyfillMatchMedia&&b.scriptUrls.polyfillMatchMedia.replace(/\?debug=true$/,""))||b.path+"/media.match.js";
f(b.loadScript(b.scriptUrls.polyfillMatchMedia,false))
}catch(d){e(d)
}});
b.polyfills.promises.push(a)
}}else{b.mustard=false
}}(window.nz.ffx,window));
(function(a,b){a.scriptUrls=a.scriptUrls||{};
b.Promise.all(a.polyfills.promises).then(function(){if(a.mustard){a.scriptUrls.app=(a.scriptUrls.app&&a.scriptUrls.app.replace(/\?debug=true$/,""))||a.path+"/app.min.js";
a.app=a.loadScript(a.scriptUrls.app,false).then(function(){a.dependencies.main.resolve();
a.log("Mustard cut and all dependencies available at "+parseInt(performance.now(),10)+" milliseconds")
})
}else{a.dependencies.main.resolve();
a.log("Mustard not cut and all dependencies available")
}})
}(window.nz.ffx,window));
(function(a,b,c){a.scriptUrls=a.scriptUrls||{};
a.dependencies.video=a.dependencies.video||{};
a.dependencies.video.promise=new b.Promise(function(d){return b.Promise.all([a.dependencies.domLoaded.promise,]).then(function(){var k=a.loadVideoHelpers;
var h=c(".bc-video-in-article");
var g=h.first();
var f=c(".bc-section-player");
var i=f.first();
var j=c("#video-channel");
var e=[];
if(h.length){e=e.concat(k.getVideoScripts({accountId:g.data("account"),playerId:g.data("player"),isLoadIma3:true}))
}if(j.length){e=e.concat(k.getVideoScripts({accountId:j.data("account"),playerId:j.data("player")}))
}if(f.length){e=e.concat(k.getVideoScripts({accountId:i.data("account"),playerId:i.data("player")}))
}if(e.length){a.videoPromises=[];
e.map(function(l){return a.videoPromises.push(a.loadScript(l,false))
});
b.Promise.all(a.videoPromises).then(function(){if(h.length){k.initDesktopArticle(c,h,a.BrightcoveVideoPlayer,b.videojs)
}if(f.length){k.initSection(c,f,a.BrightcoveVideoPlayer,b.videojs)
}if(j.length){k.initVideoChannel(a.BrightcoveVideoPlayer,a.VideoChannelPlayerHelper,b.videojs)
}d();
a.log("All video dependencies available available")
})
}})
})
}(window.nz.ffx,window,$));
...
(function(e,f,k,n,m,j,l){var b;
var i;
var c;
var d;
var h;
function a(){if(!d[h]){c.style.display="none";
i.innerHTML="Close ad";
i.classList.remove("closed");
b.classList.remove("closed");
c.classList.add("open");
f("#billboard__container").slideDown(200)
}else{c.style.display="block";
b.classList.add("closed");
i.innerHTML="Show ad";
i.classList.add("closed");
c.classList.remove("open")
}}function g(){var p=new j();
p.setHours(24);
p.setMinutes(0);
p.setSeconds(0);
p.setMilliseconds(0);
if(!d[h]){d[h]=p.getTime()
}else{delete d[h]
}n.setItem("billboardDisplay",m.stringify(d));
a()
}function o(){d=m.parse(n.getItem("billboardDisplay"))||{};
h=window.digitalData.page.pageInfo.pageID;
if(j.now()>parseInt(d[h],10)){delete d[h]
}c=l.getElementById("billboard__container");
c.style.display="block";
c.classList.add("open");
b=l.getElementById("billboard");
i=l.getElementById("billboard__toggle");
i.addEventListener("click",g);
a();
return true
}k.cmd.push(function(){k.pubads().addEventListener("slotRenderEnded",function(p){if(p.slot.getSlotElementId()==="billboard"){if(!p.isEmpty){o()
}}})
});
e.legacyBillboard={init:o,}
}(window.nz.ffx,window.jQuery,window.googletag,window.localStorage,window.JSON,window.Date,document));